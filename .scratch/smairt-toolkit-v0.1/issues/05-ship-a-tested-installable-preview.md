...
- Final review remediation on macOS/Python 3.11.15: command-level and PTY tests now cover smoke-workspace safety, license canonical manifest updates, phase-independent HPC templates, inactive capability guidance, dashboard repair confirmation, final-review license reconfirmation, and local dashboard motion. `uv run ruff format --check .`, `uv run ruff check .`, `uv run mypy src tests`, and `uv run pytest` passed (32 tests).
