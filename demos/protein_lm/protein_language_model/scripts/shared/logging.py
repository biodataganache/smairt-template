...
class TeeLogger:
    ...
    def __init__(self, log_path: Path):
        ...
        self._log_file = None
        try:
            abs_log = self.log_path.resolve()
            self._repo_root = str(abs_log.parents[3])
            self._repo_root_win = self._repo_root.replace("/", "\\")
            self._proj_root = str(abs_log.parents[2])
            self._proj_root_win = self._proj_root.replace("/", "\\")
        except Exception:
    ...
    def write(self, message: str) -> None:
        """Write to both console and log file with path normalization."""
        if self._original_stdout is not None:
            self._original_stdout.write(message)
        if self._log_file is not None:
            if self._repo_root:
                message = message.replace(self._repo_root, ".")
                message = message.replace(self._repo_root_win, ".")
            if self._proj_root:
                message = message.replace(self._proj_root, ".")
                message = message.replace(self._proj_root_win, ".")
