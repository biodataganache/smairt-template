...
- `.claude/agents/builder.md` — the real Builder sub-agent (Claude Code; spawn via the Task tool)
...
execution (debugging, big logs) to a Builder, and never trust its result without
checking the log file. See `prompts/roles/ORCHESTRATOR.md` for how to delegate.
{% endif %}
## SMAIRT workflow
